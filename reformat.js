...
//    it.links = (''+it.links).split('|').filter(it=>(it.length>0));
    ...
    if (it.links.length==0) {
      if (it.transcription || it.restricted) {
        ...
        console.log(`ALERT xid:${it.xid} (missing pdf-name in col-R) pdf:${it.links.length} npages:[${it.npages.join(',')}] jpeg:<${it.pic}>`,)
      }
      //      console.log(it);
      //      assert(it.transcription || it.restricted);
    } else {
      if (it.links.length != it.npages.length) {
        console.log(`ALERT xid:${it.xid} (missing npages in col-T) pdf:${it.links.length} npages:`,it.npages)
  //      throw 'fatal-166'
      }
    }
